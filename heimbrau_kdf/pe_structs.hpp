...
// TODO MMK : Rename this as it is poorly named.
T toLittle (T in)
{
	char *ob = (char *)&in;
...
struct export_header
{
	...
	uint32_t		name;
	uint32_t		base;
	uint32_t		numberFunctions;
	uint32_t		numberNames;
	uint32_t		addressFunctions;
	uint32_t		addressNames;
	uint32_t		addressNameOrdinals;
};
